...
void Server::run()
{
    ...
    auto server_pollfd_shared = std::make_shared<pollfd>(server_pollfd);
    _fdsList.push_back(server_pollfd_shared);

    while (true) {
        poll_fds.clear();
        
        for (auto it = _fdsList.begin(); it != _fdsList.end();) {
            auto& client = *it;
            if (!client) {
                it = _fdsList.erase(it);
                continue;
            }
...
void Server::handleNewConnection()
{
    ...
    if (client_fd < 0) {
        if (errno != EAGAIN && errno != EWOULDBLOCK) {
            if (config.debug_mode) {
                std::cerr << "[SERVER] Failed to accept new connection: " << strerror(errno) << std::endl;
            }
        }
        return;
    }
...
void Server::broadcastPackets()
{
    ...
    for (auto& client : _fdsList) {
        if (!client || client->fd == _serverFd) {
            continue;  // Ignorer le socket serveur ou les descripteurs invalides
        }
        
        // Trouver l'ID du client
        auto it = _clientIds.find(client->fd);
        if (it == _clientIds.end()) {
            continue; // Client non trouvé, passer au suivant
        }
        
        int clientId = it->second;
        
        // Vérifier que l'ID est valide
        if (clientId < 0 || clientId >= _nbClients - 1) {
            if (config.debug_mode) {
                std::cerr << "[SERVER] Invalid client ID: " << clientId << std::endl;
            }
            continue;
        }
...
void Server::removeClient(int fd)
{
    ...
    for (auto it = _fdsList.begin(); it != _fdsList.end(); ++it) {
        if (*it && (*it)->fd == fd) {
            *it = nullptr; // Marquer comme supprimé
            break;
        }
    }
    ...
    if (!clientsConnected && _gameStarted) {
        _gameStarted = false;
...
bool Server::readPacket(int client_fd, PacketModule& packetModule)
{
    ...
    memset(&tmp, 0, sizeof(tmp)); // Initialiser à zéro
...
void Server::startGame()
{
    ...
    if (clientCount < 2) {  // Less than 2 clients
        return;
    }
    
    if (config.debug_mode) {
        std::cout << "[SERVER] Starting game with " << clientCount << " players" << std::endl;
    }
    ...
    for (auto& pair : _packets) {
        if (pair.first >= 0 && pair.first < MAX_CLIENTS) {
