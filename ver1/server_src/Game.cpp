...
Game::Game() : _width(0), _height(0), _gameStarted(false), _gameOver(false) {
    ...
    // Initialiser les positions et états des joueurs
...
bool Game::loadMap(const std::string& mapFile) {
    ...
    if (_width <= 0 || _height <= 0) {
        std::cerr << "Invalid map dimensions: " << _width << "x" << _height << std::endl;
        return false;
    }
    ...
    for (int y = 0; y < _height && std::getline(file, line); ++y) {
        for (int x = 0; x < _width && x < static_cast<int>(line.length()); ++x) {
            ...
            if (_map[y][x] == START_P1 && 0 < _players.size()) {
                _players[0].x = x;
                _players[0].y = y;
                _map[y][x] = EMPTY; // On efface le marqueur pour ne pas bloquer le joueur
            } else if (_map[y][x] == START_P2 && 1 < _players.size()) {
                _players[1].x = x;
                _players[1].y = y;
                _map[y][x] = EMPTY; // On efface le marqueur pour ne pas bloquer le joueur
            }
...
void Game::checkCollisions(int playerId) {
    ...
    switch (tile) {
        case COIN:
            // Collecter la pièce
            player.score++;
            // Vérifier si l'index est dans les limites avant de modifier
            if (static_cast<int>(player.y) < _height && static_cast<int>(player.x) < _width) {
...
bool Game::isGameOver() const {
    ...
    // S'assurer qu'il y a au moins deux joueurs avant de vérifier
    if (_players.size() < 2) {
        return false;
    }
