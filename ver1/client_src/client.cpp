...
void ClientModule::Client::parseArguments(int argc, const char *argv[])
{
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg == "-h" && i + 1 < argc) {
            ...
            try {
                ...
                serverPort = 4242; // Utiliser une valeur par défaut
            }
        } else if (arg == "-d") {
            debugMode = true;
        }
    ...
    // Vérifier que les paramètres obligatoires sont définis
    if (serverIp.empty()) {
        serverIp = "127.0.0.1"; // Valeur par défaut
        std::cerr << "No server IP provided, using default: " << serverIp << std::endl;
    }
    
    if (serverPort <= 0) {
        serverPort = 4242; // Valeur par défaut
        std::cerr << "No valid port provided, using default: " << serverPort << std::endl;
    }
}
...
void ClientModule::Client::run()
{
    ...
    if (connect(fd, (struct sockaddr*)&serverAddr, sizeof(serverAddr)) < 0) {
        close(fd);
        throw std::runtime_error("Connection failed to " + serverIp + ":" + std::to_string(serverPort));
    }
...
void ClientModule::Client::startThread()
{
    ...
    try {
        // Initialiser le paquet avant de démarrer les threads
        packet = PacketModule();
...
void ClientModule::Client::runThread()
{
    ...
    // Une fois les threads terminés, fermer proprement la connexion
    if (connected) {
        close(fd);
        connected = false;
    }
...
void ClientModule::Client::receiveMap()
{
    ...
    while (!mapReceived && attempts < MAX_ATTEMPTS && connected) {
...
{
    // Créer une instance locale pour stocker les paquets reçus
    PacketModule newGameState;
    fd_set readfds;
    struct timeval tv;

    while (connected) {
        // Configurer le set de descripteurs de fichiers
        FD_ZERO(&readfds);
        FD_SET(fd, &readfds);
        
        // Configurer le timeout
        tv.tv_sec = 0;
        tv.tv_usec = 16000; // 16ms
        
        // Attendre des données avec un timeout
        int activity = select(fd + 1, &readfds, NULL, NULL, &tv);
        
        if (activity < 0) {
            if (debugMode) {
                std::cerr << "[CLIENT] Select error: " << strerror(errno) << std::endl;
            }
            if (errno != EINTR) { // Ignorer les interruptions
                connected = false;
                break;
            }
        } else if (activity > 0 && FD_ISSET(fd, &readfds)) {
            // Recevoir des données
            ssize_t recvSize = recv(fd, &newGameState, sizeof(PacketModule::Packet), 0);
        ...
        // Envoyer l'entrée du joueur au serveur
        ...
        // Utiliser send avec MSG_NOSIGNAL pour éviter SIGPIPE
        ssize_t sendSize = send(fd, &outgoingPacket.getPacket(), sizeof(PacketModule::Packet), MSG_NOSIGNAL);
